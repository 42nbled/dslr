...
def min(data, Feature):
    ...
    return sorted_data[Feature].iloc[0]

def quartile(data, Feature):
    sorted_data = data.sort_values(by=Feature)
    return sorted_data[Feature].iloc[int(len(sorted_data) * 0.25)]

def mediane(data, Feature):
    sorted_data = data.sort_values(by=Feature)
    return sorted_data[Feature].iloc[int(len(sorted_data) * 0.50)]

def T_quartile(data, Feature):
    sorted_data = data.sort_values(by=Feature)
    return sorted_data[Feature].iloc[int(len(sorted_data) * 0.75)]
...
def main():
    ...
    print(f"\t\t{Feature_1:>16} {Feature_2:>16} {Feature_3:>16} {Feature_4:>16}")
    display(count, data)
    display(mean, data)
    display(std, data)
    display(min, data)
